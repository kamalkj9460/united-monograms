//------------------------------------------------------------------------
// Code for EMB DTG package
// EMB comes before DTG and have it's own text
//------------------------------------------------------------------------
...
$(function () {
  ...
  $("#preview-monogram").on("click", function (el) {
    ...
    if (MonogramFont.indexOf("DTGMinnieMouse") != -1) {
      ...
      $("#monogramimage").html(
        "<img id='monogramImg'   class='line-item-property__field    w3-padding-48 ' style='  object-fit: fill'    alt='Monogram here'  src='//cdn.shopify.com/s/files/1/0054/6819/2886/files/ABC.png?v=1617460383'    />"
      );
    } else {
      $("#monogramimage").html(
        "<img id='monogramImg'   class='line-item-property__field    w3-padding-48 ' style='  object-fit: fill'    alt='Monogram here'  src='//cdn.shopify.com/s/files/1/0635/8561/files/ABC_white.png?11804'    />"
    ...
    } else if (MonogramFont_DTG.indexOf("DTGVinyl3") != -1) {
      $("#DTGmonogramimage").html(
        "<img id='DTGmonogramImg'   class='line-item-property__field  w3-padding-48 ' style='  object-fit: scale-down'    alt='Monogram here'  src='https://cdn.shopify.com/s/files/1/0054/6819/2886/files/DTGVinylVine3_f4dfe376-d75a-443f-a56e-1e5ff700497f.png?v=1617242346'   />"
      );
       //unable to use function due to monogram image size
       bg = $('.w3-container').css('background-color', '#ffffff');
       bg = $('#DTGmonogramimage').css('background-color', '#e5e5e5');
    } else {
      $("#DTGmonogramimage").html(
  ...
  function GenerateMonogram_DTG() {
    ...
    MonogramText_DTG = MonogramText;    //MonogramText_DTG;
      ...
      if (flagStop_DTG!="1") {       //------------------2 Letter
         ...
         $("#NextEmb").addClass('disabled').attr('disabled', 'disabled');
          ...
          //-------UserAgreement changed 06/26 #JDG            
            ...
            $('#btn add-to-cart').prop('disabled', false).removeClass('disabled');
          //-------UserAgreement changed 06/26 #JDG  
         //----Set a loader image..
 ...
 function addDivclassUserAgreement()
          {  //replacing the code due to typo error
            ...
            $('#uaID').on('change', function()
                      { 
                         if ((this.checked)&&flagStop_DTG!="1"&&MonogramText_DTG != "") {
                              $('#btn add-to-cart').prop('disabled', false);
                              $('input#MonogramText_DTG').on('click', uncheckAll);
